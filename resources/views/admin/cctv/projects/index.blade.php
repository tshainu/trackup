...
<style>
  ...
  .hero-bar { background:linear-gradient(135deg,#fd7e14,#e55a00); border-radius:16px; padding:1.25rem 1.75rem; color:#fff; margin-bottom:1.5rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1rem; }
  ...
  .filter-tab.active, .filter-tab:hover { background:#fd7e14; color:#fff; border-color:#fd7e14; }
</style>
@endpush
...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="hero-bar">
    <div>
      <h4><i class="bx bx-wrench me-2"></i>Installation Projects</h4>
      <p>Track CCTV installation progress</p>
    </div>
    <a href="{{ route('admin.cctv.projects.create') }}" class="btn btn-light btn-sm fw-600">
      <i class="bx bx-plus me-1"></i> New Project
    </a>
  </div>

  <div class="stat-grid">
    <div class="stat-card sc-blue"><div class="stat-icon"><i class="bx bx-wrench"></i></div><div><div class="stat-num">{{ $stats['total'] ?? 0 }}</div><div class="stat-lbl">Total</div></div></div>
    <div class="stat-card sc-orange"><div class="stat-icon"><i class="bx bx-time"></i></div><div><div class="stat-num">{{ $stats['in_progress'] ?? 0 }}</div><div class="stat-lbl">In Progress</div></div></div>
    ...
    <div class="stat-card sc-purple"><div class="stat-icon"><i class="bx bx-calendar"></i></div><div><div class="stat-num">{{ $stats['scheduled'] ?? 0 }}</div><div class="stat-lbl">Scheduled</div></div></div>
  </div>
    ...
    <div class="card-body pb-2">
      <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between">
        <div class="filter-tabs">
          <a href="{{ route('admin.cctv.projects.index') }}" class="filter-tab {{ !request('status') ? 'active' : '' }}">All</a>
          @foreach(['scheduled','in_progress','completed','on_hold','cancelled'] as $s)
            <a href="{{ route('admin.cctv.projects.index', ['status'=>$s]) }}" class="filter-tab {{ request('status')===$s ? 'active' : '' }}">{{ ucwords(str_replace('_',' ',$s)) }}</a>
          @endforeach
        </div>
  ...
  <div class="card border-0 shadow-sm">
    <div class="table-responsive">
      <table class="table table-hover align-middle mb-0">
        ...
        <tbody>
          ...
          <tr>
            <td><span class="fw-700 text-primary font-monospace">{{ $project->project_no }}</span></td>
            <td><div class="fw-600">{{ $project->customer_name }}</div></td>
            <td class="font-monospace small">{{ $project->mobile }}</td>
            ...
            <td>
              ...
              <span class="badge bg-label-{{ $sc }}">{{ ucwords(str_replace('_',' ',$project->status)) }}</span>
            </td>
            <td>{{ $project->start_date ? \Carbon\Carbon::parse($project->start_date)->format('d M Y') : '—' }}</td>
            <td>{{ $project->contract_amount ? number_format($project->contract_amount, 2) : '—' }}</td>
            <td class="text-end">
              <a href="{{ route('admin.cctv.projects.show', $project) }}" class="btn btn-sm btn-outline-primary py-1 px-2"><i class="bx bx-show"></i></a>
              <a href="{{ route('admin.cctv.projects.edit', $project) }}" class="btn btn-sm btn-outline-secondary py-1 px-2"><i class="bx bx-edit"></i></a>
            </td>
          </tr>
          @empty
    ...
    <div class="card-footer bg-transparent d-flex justify-content-end">{{ $projects->withQueryString()->links() }}</div>
    @endif
  </div>
</div>
@endsection
