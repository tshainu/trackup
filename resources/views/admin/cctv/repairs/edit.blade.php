...
@section('title', 'Edit Repair – ' . $repair->repair_no)
...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="hero-bar">
    <a href="{{ route('admin.cctv.repairs.show', $repair) }}" class="back-btn"><i class="bx bx-arrow-back"></i></a>
    <div><h4>Edit Repair – {{ $repair->repair_no }}</h4><p>{{ $repair->customer_name }}</p></div>
  </div>

  <form method="POST" action="{{ route('admin.cctv.repairs.update', $repair) }}">
    @csrf @method('PUT')
    <div class="row g-3">
      <div class="col-lg-8">
        <div class="card form-card">
          ...
          <div class="card-body row g-3">
            ...
            <div class="col-md-6">
              ...
              <input type="text" name="mobile" class="form-control" value="{{ old('mobile', $repair->mobile) }}">
            </div>
          </div>
        </div>
        ...
        <div class="card form-card">
          ...
          <div class="card-body row g-3">
            <div class="col-md-4">
              <label class="form-label fw-600">Device Type</label>
              <select name="device_type" class="form-select">
                <option value="">— Select —</option>
                @foreach(['IP Camera','Analog Camera','DVR','NVR','PTZ Camera','HDD','Monitor','Other'] as $t)
                  <option value="{{ $t }}" {{ old('device_type',$repair->device_type)===$t?'selected':'' }}>{{ $t }}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-4">
              <label class="form-label fw-600">Brand</label>
              <input type="text" name="brand" class="form-control" value="{{ old('brand', $repair->brand) }}">
            </div>
            <div class="col-md-4">
              <label class="form-label fw-600">Model</label>
              <input type="text" name="model" class="form-control" value="{{ old('model', $repair->model) }}">
            </div>
            <div class="col-md-6">
              <label class="form-label fw-600">Serial No</label>
              <input type="text" name="serial_no" class="form-control font-monospace" value="{{ old('serial_no', $repair->serial_no) }}">
            </div>
            <div class="col-md-6">
              <label class="form-label fw-600">Status</label>
              <select name="status" class="form-select">
                @foreach(['pending','in_progress','completed','delivered','cancelled'] as $s)
                  <option value="{{ $s }}" {{ old('status',$repair->status)===$s?'selected':'' }}>{{ ucwords(str_replace('_',' ',$s)) }}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-6">
              <label class="form-label fw-600">Technician</label>
              <input type="text" name="technician_name" class="form-control" value="{{ old('technician_name', $repair->technician_name) }}">
            </div>
            <div class="col-md-6">
              <label class="form-label fw-600">Estimated Delivery</label>
              <input type="date" name="estimated_delivery" class="form-control" value="{{ old('estimated_delivery', $repair->estimated_delivery ? \Carbon\Carbon::parse($repair->estimated_delivery)->format('Y-m-d') : '') }}">
            </div>
            <div class="col-12">
              <label class="form-label fw-600">Issue Description</label>
              <textarea name="issue_description" class="form-control" rows="3">{{ old('issue_description', $repair->issue_description) }}</textarea>
            </div>
            <div class="col-md-4">
              <label class="form-label fw-600">Repair Charge (Rs.)</label>
              <input type="number" name="repair_charge" step="0.01" class="form-control" value="{{ old('repair_charge', $repair->repair_charge ?? 0) }}">
            </div>
            <div class="col-md-4">
              <label class="form-label fw-600">Parts Cost (Rs.)</label>
              <input type="number" name="parts_cost" step="0.01" class="form-control" value="{{ old('parts_cost', $repair->parts_cost ?? 0) }}">
            </div>
            <div class="col-md-4">
              <label class="form-label fw-600">Advance (Rs.)</label>
              <input type="number" name="advance_paid" step="0.01" class="form-control" value="{{ old('advance_paid', $repair->advance_paid ?? 0) }}">
            </div>
            <div class="col-12">
              <label class="form-label fw-600">Repair Notes</label>
              <textarea name="repair_notes" class="form-control" rows="2">{{ old('repair_notes', $repair->repair_notes) }}</textarea>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card form-card">
          ...
          <div class="card-body">
            <div class="mb-3 p-3 rounded" style="background:#f8f9fa;">
              <div class="small text-muted fw-600">Repair No</div>
              <div class="fw-700 font-monospace text-primary">{{ $repair->repair_no }}</div>
              <div class="small text-muted mt-1">{{ $repair->created_at->format('d M Y') }}</div>
            </div>
            <div class="d-grid gap-2">
              <button type="submit" class="btn btn-primary"><i class="bx bx-save me-1"></i> Update</button>
              <a href="{{ route('admin.cctv.repairs.show', $repair) }}" class="btn btn-outline-secondary">Cancel</a>
            </div>
          </div>
        </div>
      </div>
