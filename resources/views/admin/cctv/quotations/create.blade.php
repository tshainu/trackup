...
<style>
  ...
  .items-table .del-row { background:none; border:none; color:#ea5455; padding:4px 8px; border-radius:6px; }
  .items-table .del-row:hover { background:#fdeaea; }
...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <form method="POST" action="{{ route('admin.cctv.quotations.store') }}" id="quotationForm">
    @csrf
    <div class="row g-3">
      <div class="col-lg-8">
        {{-- Customer --}}
        ...
        {{-- Line Items --}}
        <div class="card form-card">
          <div class="card-header"><div class="section-icon"><i class="bx bx-list-ul"></i></div> Items / Products</div>
          <div class="card-body">
            <div class="table-responsive">
              <table class="table items-table align-middle" id="itemsTable">
                ...
                <tbody id="itemsBody">
                  <tr class="item-row">
                    <td><input type="text" name="items[0][description]" class="form-control form-control-sm" placeholder="Camera / DVR / Cable…" required></td>
                    <td><input type="number" name="items[0][qty]" class="form-control form-control-sm qty-input" value="1" min="1" style="width:70px"></td>
                    <td><input type="number" name="items[0][unit_price]" class="form-control form-control-sm price-input" value="0" step="0.01" style="width:110px"></td>
                    <td><span class="row-total fw-600">0.00</span></td>
                    <td><button type="button" class="del-row" onclick="removeRow(this)"><i class="bx bx-trash"></i></button></td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr class="total-row">
                    <td colspan="3" class="text-end pe-3">Sub Total</td>
                    <td><span id="subTotal">0.00</span></td>
                    <td></td>
                  </tr>
                  <tr>
                    <td colspan="2" class="text-end pe-3">Discount (Rs.)</td>
                    <td><input type="number" name="discount_amount" id="discountInput" class="form-control form-control-sm" value="0" step="0.01"></td>
                    <td><span id="discountDisplay">0.00</span></td>
                    <td></td>
                  </tr>
                  <tr class="total-row">
                    <td colspan="3" class="text-end pe-3 fs-6">Grand Total</td>
                    <td><span id="grandTotal" class="text-primary fs-6">0.00</span></td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
...
<script>
  ...
  function addRow() {
    ...
    tr.innerHTML = `
      <td><input type="text" name="items[${rowIndex}][description]" class="form-control form-control-sm" placeholder="Item…"></td>
      <td><input type="number" name="items[${rowIndex}][qty]" class="form-control form-control-sm qty-input" value="1" min="1" style="width:70px"></td>
      <td><input type="number" name="items[${rowIndex}][unit_price]" class="form-control form-control-sm price-input" value="0" step="0.01" style="width:110px"></td>
      <td><span class="row-total fw-600">0.00</span></td>
      <td><button type="button" class="del-row" onclick="removeRow(this)"><i class="bx bx-trash"></i></button></td>`;
  ...
  function removeRow(btn) {
    ...
    if (rows.length > 1) { btn.closest('.item-row').remove(); calcTotals(); }
  }
  ...
  // Keep hidden field in sync when user types name manually
